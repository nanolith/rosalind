/**
 * \file test/bootstrap/rcc_scanner.cpp
 *
 * \brief Unit tests for \ref rcc_scanner.
 *
 * See LICENSE.txt for copyright and license information.
 */

#include <minunit/minunit.h>
#include <rosalind_bootstrap/parser.h>
#include <string.h>

#include "bootstrap/parser_internal.h"

TEST_SUITE(rcc_scanner);

/**
 * \brief Test that we can scan the EOF token.
 */
TEST(EOF_simple)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the EOF token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_EOF == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(0 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(1 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we skip whitespace to read EOF from a blank string.
 */
TEST(EOF_whitespace_string)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t \n";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the EOF token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_EOF == details.type);
    TEST_EXPECT(5 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(2 == details.begin_line);
    TEST_EXPECT(2 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(1 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an AND token.
 */
TEST(AND_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "AND \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the AND token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_AND
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_AND == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that AND falls back to identifier.
 */
TEST(AND_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ANDS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that AND is advanced, even with the identifier fallback.
 */
TEST(AND_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "AND ANDS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an AND keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_AND
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read an EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an ARRAY token.
 */
TEST(ARRAY_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ARRAY \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the ARRAY token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_ARRAY
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_ARRAY == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that ARRAY falls back to identifier.
 */
TEST(ARRAY_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ARRAYS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that ARRAY advances the scanner.
 */
TEST(ARRAY_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ARRAY ARRAYS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an ARRAY keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_ARRAY
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a BEGIN token.
 */
TEST(BEGIN_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "BEGIN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the BEGIN token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_BEGIN
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_BEGIN == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that BEGIN complete falls back to identifier.
 */
TEST(BEGIN_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "BEGINA \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that BEGIN advances the scanner.
 */
TEST(BEGIN_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "BEGIN BEGINA \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a BEGIN keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_BEGIN
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a CASE token.
 */
TEST(CASE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "CASE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the CASE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_CASE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_CASE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that CASE complete falls back to identifier.
 */
TEST(CASE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "CASE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that CASE advances the scanner.
 */
TEST(CASE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "CASE CASE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a CASE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_CASE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a CONST token.
 */
TEST(CONST_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "CONST \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the CONST token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_CONST
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_CONST == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that CONST complete falls back to identifier.
 */
TEST(CONST_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "CONSTANT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that CONST advances the scanner.
 */
TEST(CONST_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "CONST CONSTANT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a CONST keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_CONST
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a DIV token.
 */
TEST(DIV_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "DIV \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the DIV token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_DIV
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_DIV == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that DIV complete falls back to identifier.
 */
TEST(DIV_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "DIVIDE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that DIV advances the scanner.
 */
TEST(DIV_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "DIV DIVIDE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a DIV keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_DIV
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a DO token.
 */
TEST(DO_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "DO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the DO token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_DO
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_DO == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that DO complete falls back to identifier.
 */
TEST(DO_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "DONT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that DO advances the scanner.
 */
TEST(DO_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "DO DONT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a DO keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_DO
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an ELSE token.
 */
TEST(ELSE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ELSE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the ELSE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_ELSE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_ELSE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that ELSE complete falls back to identifier.
 */
TEST(ELSE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ELSEIF \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that ELSE advances the scanner.
 */
TEST(ELSE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ELSE ELSEIF \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an ELSE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_ELSE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an END token.
 */
TEST(END_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "END \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the END token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_END
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_END == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that END complete falls back to identifier.
 */
TEST(END_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "ENDIAN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that END advances the scanner.
 */
TEST(END_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "END ENDIAN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an END keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_END
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a FOR token.
 */
TEST(FOR_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "FOR \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the FOR token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_FOR
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_FOR == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that FOR complete falls back to identifier.
 */
TEST(FOR_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "FORTH \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that FOR advances the scanner.
 */
TEST(FOR_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "FOR FORTH \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a FOR keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_FOR
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a FUNCTION token.
 */
TEST(FUNCTION_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "FUNCTION \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the FUNCTION token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_FUNCTION
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_FUNCTION == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that FUNCTION complete falls back to identifier.
 */
TEST(FUNCTION_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "FUNCTIONAL \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT( 9 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(10 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that FUNCTION advances the scanner.
 */
TEST(FUNCTION_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "FUNCTION FUNCTIONAL \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a FUNCTION keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_FUNCTION
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a GOTO token.
 */
TEST(GOTO_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "GOTO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the GOTO token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_GOTO
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_GOTO == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that GOTO complete falls back to identifier.
 */
TEST(GOTO_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "GOTO2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that GOTO advances the scanner.
 */
TEST(GOTO_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "GOTO GOTO2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a GOTO keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_GOTO
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a HIDE token.
 */
TEST(HIDE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "HIDE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the HIDE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_HIDE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_HIDE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that HIDE complete falls back to identifier.
 */
TEST(HIDE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "HIDDEN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that HIDE advances the scanner.
 */
TEST(HIDE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "HIDE HIDDEN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a HIDE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_HIDE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an IF token.
 */
TEST(IF_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IF \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the IF token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IF
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_IF == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IF falls back to identifier.
 */
TEST(IF_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IF2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IF advances the scanner.
 */
TEST(IF_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IF IF2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an IF keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IF
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an IMPLEMENT token.
 */
TEST(IMPLEMENT_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPLEMENT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the IMPLEMENT token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IMPLEMENT
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_IMPLEMENT == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(8 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(9 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IMPLEMENT falls back to identifier.
 */
TEST(IMPLEMENT_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPLEMENTATION \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(13 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(14 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IMPLEMENT advances the scanner.
 */
TEST(IMPLEMENT_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPLEMENT IMPLEMENTATION \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an IMPLEMENT keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IMPLEMENT
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an IMPLEMENTS token.
 */
TEST(IMPLEMENTS_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPLEMENTS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the IMPLEMENTS token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IMPLEMENTS
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_IMPLEMENTS == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT( 9 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(10 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IMPLEMENTS falls back to identifier.
 */
TEST(IMPLEMENTS_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPLEMENTS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(10 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(11 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IMPLEMENTS advances the scanner.
 */
TEST(IMPLEMENTS_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPLEMENTS IMPLEMENTS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an IMPLEMENTS keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IMPLEMENTS
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an IMPORT token.
 */
TEST(IMPORT_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPORT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the IMPORT token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IMPORT
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_IMPORT == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IMPORT falls back to identifier.
 */
TEST(IMPORT_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPORTED \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IMPORT advances the scanner.
 */
TEST(IMPORT_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IMPORT IMPORTED \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an IMPORT keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IMPORT
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an IN token.
 */
TEST(IN_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the IN token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IN
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_IN == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IN falls back to identifier.
 */
TEST(IN_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IN2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that IN advances the scanner.
 */
TEST(IN_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "IN IN2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an IN keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_IN
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an INT token.
 */
TEST(INT_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the INT token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_INT
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_INT == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that INT falls back to identifier.
 */
TEST(INT_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INTO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that INT advances the scanner.
 */
TEST(INT_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INT INTO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an INT keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_INT
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an INTERFACE token.
 */
TEST(INTERFACE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INTERFACE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the INTERFACE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_INTERFACE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_INTERFACE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(8 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(9 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that INTERFACE falls back to identifier.
 */
TEST(INTERFACE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INTERFACES \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT( 9 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(10 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that INTERFACE advances the scanner.
 */
TEST(INTERFACE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INTERFACE INTERFACES \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an INTERFACE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_INTERFACE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an INVARIANTS token.
 */
TEST(INVARIANTS_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INVARIANTS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the INVARIANTS token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_INVARIANTS
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_INVARIANTS == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT( 9 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(10 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that INVARIANTS falls back to identifier.
 */
TEST(INVARIANTS_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INVARIANTS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(10 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(11 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that INVARIANTS advances the scanner.
 */
TEST(INVARIANTS_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "INVARIANTS INVARIANTS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an INVARIANTS keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_INVARIANTS
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a MOD token.
 */
TEST(MOD_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "MOD \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the MOD token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_MOD
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_MOD == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that MOD falls back to identifier.
 */
TEST(MOD_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "MODE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that MOD advances the scanner.
 */
TEST(MOD_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "MOD MODE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a MOD keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_MOD
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a MODULE token.
 */
TEST(MODULE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "MODULE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the MODULE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_MODULE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_MODULE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that MODULE falls back to identifier.
 */
TEST(MODULE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "MODULES \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that MODULE advances the scanner.
 */
TEST(MODULE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "MODULE MODULES \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a MODULE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_MODULE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a NIL token.
 */
TEST(NIL_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "NIL \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the NIL token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_NIL
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_NIL == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that NIL falls back to identifier.
 */
TEST(NIL_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "NILS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that NIL advances the scanner.
 */
TEST(NIL_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "NIL NILS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a NIL keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_NIL
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a NOT token.
 */
TEST(NOT_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "NOT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the NOT token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_NOT
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_NOT == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that NOT falls back to identifier.
 */
TEST(NOT_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "NOTS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that NOT advances the scanner.
 */
TEST(NOT_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "NOT NOTS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a NOT keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_NOT
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an OF token.
 */
TEST(OF_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OF \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the OF token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_OF
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_OF == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that OF falls back to identifier.
 */
TEST(OF_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OF2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that OF advances the scanner.
 */
TEST(OF_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OF OF2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an OF keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_OF
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an OPAQUE token.
 */
TEST(OPAQUE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OPAQUE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the OPAQUE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_OPAQUE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_OPAQUE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that OPAQUE falls back to identifier.
 */
TEST(OPAQUE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OPAQUE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that OPAQUE advances the scanner.
 */
TEST(OPAQUE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OPAQUE OPAQUE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an OPAQUE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_OPAQUE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an OR token.
 */
TEST(OR_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OR \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the OR token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_OR
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_OR == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that OR falls back to identifier.
 */
TEST(OR_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OR2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that OR advances the scanner.
 */
TEST(OR_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "OR OR2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an OR keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_OR
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a POINTER token.
 */
TEST(POINTER_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "POINTER \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the POINTER token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_POINTER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_POINTER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that POINTER falls back to identifier.
 */
TEST(POINTER_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "POINTERS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that POINTER advances the scanner.
 */
TEST(POINTER_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "POINTER POINTERS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a POINTER keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_POINTER
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a POSTCONDITIONS token.
 */
TEST(POSTCONDITIONS_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "POSTCONDITIONS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the POSTCONDITIONS token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_POSTCONDITIONS
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_POSTCONDITIONS == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(13 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(14 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that POSTCONDITIONS falls back to identifier.
 */
TEST(POSTCONDITIONS_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "POSTCONDITIONS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(14 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(15 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that POSTCONDITIONS advances the scanner.
 */
TEST(POSTCONDITIONS_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "POSTCONDITIONS POSTCONDITIONS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a POSTCONDITIONS keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_POSTCONDITIONS
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a PRECONDITIONS token.
 */
TEST(PRECONDITIONS_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PRECONDITIONS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the PRECONDITIONS token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PRECONDITIONS
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_PRECONDITIONS == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(12 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(13 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PRECONDITIONS falls back to identifier.
 */
TEST(PRECONDITIONS_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PRECONDITIONS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT(13 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(14 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PRECONDITIONS advances the scanner.
 */
TEST(PRECONDITIONS_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PRECONDITIONS PRECONDITIONS2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a PRECONDITIONS keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PRECONDITIONS
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a PRIVATE token.
 */
TEST(PRIVATE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PRIVATE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the PRIVATE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PRIVATE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_PRIVATE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PRIVATE falls back to identifier.
 */
TEST(PRIVATE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PRIVATE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PRIVATE advances the scanner.
 */
TEST(PRIVATE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PRIVATE PRIVATE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a PRIVATE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PRIVATE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a PROCEDURE token.
 */
TEST(PROCEDURE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PROCEDURE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the PROCEDURE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PROCEDURE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_PROCEDURE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(8 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(9 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PROCEDURE falls back to identifier.
 */
TEST(PROCEDURE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PROCEDURE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT( 0 == details.begin_index);
    TEST_EXPECT( 9 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 1 == details.begin_col);
    TEST_EXPECT(10 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PROCEDURE advances the scanner.
 */
TEST(PROCEDURE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PROCEDURE PROCEDURE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a PROCEDURE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PROCEDURE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a PUBLIC token.
 */
TEST(PUBLIC_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PUBLIC \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the PUBLIC token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PUBLIC
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_PUBLIC == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PUBLIC falls back to identifier.
 */
TEST(PUBLIC_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PUBLIC2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that PUBLIC advances the scanner.
 */
TEST(PUBLIC_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "PUBLIC PUBLIC2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a PUBLIC keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_PUBLIC
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a RECORD token.
 */
TEST(RECORD_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "RECORD \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the RECORD token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_RECORD
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_RECORD == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that RECORD falls back to identifier.
 */
TEST(RECORD_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "RECORD2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that RECORD advances the scanner.
 */
TEST(RECORD_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "RECORD RECORD2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a RECORD keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_RECORD
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a REVEAL token.
 */
TEST(REVEAL_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "REVEAL \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the REVEAL token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_REVEAL
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_REVEAL == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that REVEAL falls back to identifier.
 */
TEST(REVEAL_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "REVEAL2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that REVEAL advances the scanner.
 */
TEST(REVEAL_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "REVEAL REVEAL2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a REVEAL keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_REVEAL
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a SET token.
 */
TEST(SET_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SET \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the SET token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_SET
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_SET == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that SET falls back to identifier.
 */
TEST(SET_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SET2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that SET advances the scanner.
 */
TEST(SET_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SET SET2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a SET keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_SET
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an SHL token.
 */
TEST(SHL_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SHL \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the SHL token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_SHL
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_SHL == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that SHL falls back to identifier.
 */
TEST(SHL_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SHL2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that SHL advances the scanner.
 */
TEST(SHL_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SHL SHL2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a SHL keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_SHL
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read an SHR token.
 */
TEST(SHR_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SHR \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the SHR token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_SHR
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_SHR == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that SHR falls back to identifier.
 */
TEST(SHR_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SHR2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that SHR advances the scanner.
 */
TEST(SHR_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "SHR SHR2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a SHR keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_SHR
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a STRING token.
 */
TEST(STRING_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "STRING \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the STRING token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_STRING
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_STRING == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that STRING falls back to identifier.
 */
TEST(STRING_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "STRING2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that STRING advances the scanner.
 */
TEST(STRING_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "STRING STRING2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a STRING keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_STRING
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a THEN token.
 */
TEST(THEN_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "THEN \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the THEN token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_THEN
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_THEN == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that THEN falls back to identifier.
 */
TEST(THEN_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "THEN2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that THEN advances the scanner.
 */
TEST(THEN_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "THEN THEN2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a THEN keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_THEN
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a TO token.
 */
TEST(TO_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "TO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the TO token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_TO
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_TO == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that TO falls back to identifier.
 */
TEST(TO_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "TOO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that TO advances the scanner.
 */
TEST(TO_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "TO TOO \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a TO keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_TO
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a TYPE token.
 */
TEST(TYPE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "TYPE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the TYPE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_TYPE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_TYPE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that TYPE falls back to identifier.
 */
TEST(TYPE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "TYPES \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that TYPE advances the scanner.
 */
TEST(TYPE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "TYPE TYPES \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a TYPE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_TYPE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a UINT token.
 */
TEST(UINT_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "UINT \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the UINT token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_UINT
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_UINT == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that UINT complete falls back to identifier.
 */
TEST(UINT_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "UINT2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that UINT advances the scanner.
 */
TEST(UINT_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "UINT UINT2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a UINT keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_UINT
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a VAR token.
 */
TEST(VAR_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "VAR \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the VAR token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_VAR
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_VAR == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that VAR falls back to identifier.
 */
TEST(VAR_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "VARS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that VAR advances the scanner.
 */
TEST(VAR_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "VAR VARS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a VAR keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_VAR
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a WHERE token.
 */
TEST(WHERE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "WHERE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the WHERE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_WHERE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_WHERE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that WHERE falls back to identifier.
 */
TEST(WHERE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "WHERE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that WHERE advances the scanner.
 */
TEST(WHERE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "WHERE WHERE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a WHERE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_WHERE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a WHILE token.
 */
TEST(WHILE_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "WHILE \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the WHILE token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_WHILE
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_WHILE == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that WHILE falls back to identifier.
 */
TEST(WHILE_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "WHILE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that WHILE advances the scanner.
 */
TEST(WHILE_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "WHILE WHILE2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a WHILE keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_WHILE
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can read a XOR token.
 */
TEST(XOR_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "XOR \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read the XOR token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_XOR
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_KEYWORD_XOR == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that XOR falls back to identifier.
 */
TEST(XOR_identifier_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "XORS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that XOR advances the scanner.
 */
TEST(XOR_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "XOR XORS \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an XOR keyword. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_KEYWORD_XOR
            == rcc_scanner_read_token(&details, scanner));

    /* read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a generic identifier.
 */
TEST(identifier_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t foo2 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an identifier. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_IDENTIFIER
            == rcc_scanner_read_token(&details, scanner));

    TEST_EXPECT(RCC_TOKEN_TYPE_IDENTIFIER == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a simple number.
 */
TEST(simple_number)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(123 == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a large number.
 */
TEST(large_number)
{
    const uintmax_t maxval = UINTMAX_MAX;
    char INPUT[1024];
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;

    sprintf(INPUT, "%ju", maxval);

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT((strlen(INPUT)-1) == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(strlen(INPUT) == details.end_col);

    TEST_EXPECT(details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(maxval == details.primitive_val.integerv.val.unsignedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Out-of-range error.
 */
TEST(too_large_number)
{
    const uintmax_t maxval = UINTMAX_MAX;
    char INPUT[1024];
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;

    sprintf(INPUT, "%ju999", maxval);

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number fails. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_BAD_INPUT
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_BAD_INPUT == details.type);
    TEST_EXPECT(0 == details.begin_index);
    TEST_EXPECT((strlen(INPUT)-1) == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(1 == details.begin_col);
    TEST_EXPECT(strlen(INPUT) == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a negative number.
 */
TEST(negative_number)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t -123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);

    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(-123 == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that number advances the scanner.
 */
TEST(number_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "123 -123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that the negative number scanner falls back to the minus token.
 */
TEST(negative_number_scan_minus_token_fallback)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t - \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a minus token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_MINUS
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_MINUS == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a minus token advances the scanner.
 */
TEST(minus_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "- 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a minus token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_MINUS
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a hex number.
 */
TEST(hex_number_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t $12BADF \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT( 4 == details.begin_index);
    TEST_EXPECT(10 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 5 == details.begin_col);
    TEST_EXPECT(11 == details.end_col);


    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(0x12BADF == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a negative hex number.
 */
TEST(hex_number_negative)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t -$12BADF \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT( 4 == details.begin_index);
    TEST_EXPECT(11 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 5 == details.begin_col);
    TEST_EXPECT(12 == details.end_col);


    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(-0x12BADF == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan an octal number.
 */
TEST(octal_number_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t &31 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);


    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(031 == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a negative octal number.
 */
TEST(octal_number_negative)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t -&31 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(7 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(8 == details.end_col);


    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(-031 == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a binary number.
 */
TEST(binary_number_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t %0111 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(8 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(9 == details.end_col);


    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(7 == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a negative binary number.
 */
TEST(binary_number_negative)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t -%0111 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a number. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NUMBER == details.type);
    TEST_EXPECT( 4 == details.begin_index);
    TEST_EXPECT( 9 == details.end_index);
    TEST_EXPECT( 1 == details.begin_line);
    TEST_EXPECT( 1 == details.end_line);
    TEST_EXPECT( 5 == details.begin_col);
    TEST_EXPECT(10 == details.end_col);


    TEST_EXPECT(!details.primitive_val.integerv.is_unsigned);
    TEST_EXPECT(-7 == details.primitive_val.integerv.val.signedv);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a plus token.
 */
TEST(plus_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t + \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a plus. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_PLUS
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_PLUS == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a plus token advances the scanner.
 */
TEST(plus_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "+ 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a plus token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_PLUS
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a star token.
 */
TEST(star_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t * \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a star. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_STAR
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_STAR == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a star token advances the scanner.
 */
TEST(star_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "* 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a star token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_STAR
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a slash token.
 */
TEST(slash_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t / \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a slash. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_SLASH
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_SLASH == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a slash token advances the scanner.
 */
TEST(slash_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "/ 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a slash token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_SLASH
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan an equal token.
 */
TEST(equal_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t = \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an equal. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_EQUAL == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that an equal token advances the scanner.
 */
TEST(equal_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "= 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an equal token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a less-than-equal token.
 */
TEST(less_than_equal_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t <= \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a less-than-equal. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_LESS_THAN_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_LESS_THAN_EQUAL == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a less-than-equal token advances the scanner.
 */
TEST(less_than_equal_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "<= 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a less-than-equal token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_LESS_THAN_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a less-than token.
 */
TEST(less_than_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t < \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a less-than. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_LESS_THAN
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_LESS_THAN == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a less-than token advances the scanner.
 */
TEST(less_than_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "< 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a less-than token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_LESS_THAN
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a not-equal token.
 */
TEST(not_equal_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t <> \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a not-equal. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NOT_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_NOT_EQUAL == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a not-equal token advances the scanner.
 */
TEST(not_equal_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "<> 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a not-equal token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NOT_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a greater-than-equal token.
 */
TEST(greater_than_equal_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t >= \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a greater-than-equal. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_GREATER_THAN_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_GREATER_THAN_EQUAL == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a greater-than-equal token advances the scanner.
 */
TEST(greater_than_equal_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = ">= 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a greater-than-equal token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_GREATER_THAN_EQUAL
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a greater-than token.
 */
TEST(greater_than_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t > \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a greater-than. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_GREATER_THAN
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_GREATER_THAN == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that a greater-than token advances the scanner.
 */
TEST(greater_than_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "> 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a greater-than token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_GREATER_THAN
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan an assignment token.
 */
TEST(assignment_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t := \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read an assignment. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_ASSIGNMENT
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_ASSIGNMENT == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that an assignment token advances the scanner.
 */
TEST(assignment_advances_scanner)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = ":= 123 \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read an assignment token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_ASSIGNMENT
            == rcc_scanner_read_token(&details, scanner));

    /* read a number token. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_NUMBER
            == rcc_scanner_read_token(&details, scanner));

    /* read EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that we can scan a colon token.
 */
TEST(colon_happy_path)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = "  \t : \t ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* attempt to read a colon. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_COLON
            == rcc_scanner_read_token(&details, scanner));

    TEST_ASSERT(RCC_TOKEN_TYPE_COLON == details.type);
    TEST_EXPECT(4 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(5 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* clean up. */
    rcc_scanner_release(scanner);
}

/**
 * \brief Test that comments are ignored as whitespace.
 */
TEST(comment_ignored)
{
    rcc_scanner* scanner = nullptr;
    rcc_token_details details;
    const char* INPUT = ": { this comment is ignored. } +";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == rcc_scanner_create(&scanner, INPUT));

    /* read a colon. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_COLON
            == rcc_scanner_read_token(&details, scanner));

    /* the next token should be a plus. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_PLUS
            == rcc_scanner_read_token(&details, scanner));

    /* the final token should be an EOF. */
    TEST_ASSERT(
        RCC_TOKEN_TYPE_EOF
            == rcc_scanner_read_token(&details, scanner));

    /* clean up. */
    rcc_scanner_release(scanner);
}
