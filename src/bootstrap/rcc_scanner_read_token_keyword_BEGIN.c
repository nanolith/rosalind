/**
 * \file bootstrap/rcc_scanner_read_token_keyword_BEGIN.c
 *
 * \brief Read a BEGIN token.
 *
...
{
    /* for this function, the token must begin with 'B'. */
    if ('B' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_BEGIN(details, scanner);
}
