/**
 * \file bootstrap/rcc_scanner_read_token_keyword_W_STAR.c
 *
 * \brief Read all of the tokens beginning with an W.
 *
...
{
    /* for this function, the token must begin with a 'W'. */
    if ('W' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_W_STAR(details, scanner);
}
