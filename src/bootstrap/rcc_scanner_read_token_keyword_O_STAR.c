/**
 * \file bootstrap/rcc_scanner_read_token_keyword_O_STAR.c
 *
 * \brief Read all of the tokens beginning with an O.
 *
...
{
    /* for this function, the token must begin with an 'O'. */
    if ('O' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_O_STAR(details, scanner);
}
