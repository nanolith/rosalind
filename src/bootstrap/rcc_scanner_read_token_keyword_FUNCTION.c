/**
 * \file bootstrap/rcc_scanner_read_token_keyword_FUNCTION.c
 *
 * \brief Read a FUNCTION token.
 *
...
{
    /* for this function, the token must begin with an 'F'. */
    if ('F' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_FUNCTION(details, scanner);
}
