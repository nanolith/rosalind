/**
 * \file bootstrap/rcc_scanner_read_token_keyword_P_STAR.c
 *
 * \brief Read all of the tokens beginning with a P.
 *
...
{
    /* for this function, the token must begin with a 'P'. */
    if ('P' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_P_STAR(details, scanner);
}
