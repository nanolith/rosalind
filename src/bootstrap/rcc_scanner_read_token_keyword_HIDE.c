/**
 * \file bootstrap/rcc_scanner_read_token_keyword_HIDE.c
 *
 * \brief Read a HIDE token.
 *
...
{
    /* for this function, the token must begin with an 'H'. */
    if ('H' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_HIDE(details, scanner);
}
