/**
 * \file bootstrap/rcc_scanner_read_token_keyword_VAR.c
 *
 * \brief Read a VAR token.
 *
...
{
    /* for this function, the token must begin with a 'V'. */
    if ('V' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_VAR(details, scanner);
}
