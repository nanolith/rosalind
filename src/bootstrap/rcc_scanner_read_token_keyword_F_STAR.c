/**
 * \file bootstrap/rcc_scanner_read_token_keyword_F_STAR.c
 *
 * \brief Read all of the tokens beginning with an F.
 *
...
{
    /* for this function, the token must begin with a 'F'. */
    if ('F' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_F_STAR(details, scanner);
}
