/**
 * \file bootstrap/rcc_scanner_read_token_keyword_R_STAR.c
 *
 * \brief Read all of the tokens beginning with an R.
 *
...
{
    /* for this function, the token must begin with an 'R'. */
    if ('R' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_R_STAR(details, scanner);
}
