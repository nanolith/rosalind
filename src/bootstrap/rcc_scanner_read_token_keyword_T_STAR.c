/**
 * \file bootstrap/rcc_scanner_read_token_keyword_T_STAR.c
 *
 * \brief Read all of the tokens beginning with an T.
 *
...
{
    /* for this function, the token must begin with a 'T'. */
    if ('T' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_T_STAR(details, scanner);
}
