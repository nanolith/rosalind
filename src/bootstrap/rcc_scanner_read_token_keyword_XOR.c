/**
 * \file bootstrap/rcc_scanner_read_token_keyword_XOR.c
 *
 * \brief Read a XOR token.
 *
...
{
    /* for this function, the token must begin with an 'X'. */
    if ('X' != *(scanner->input))
    {
    ...
    return rcc_scanner_complete_token_keyword_XOR(details, scanner);
}
